...
class Lidar:
    def __init__(self, n_rays=360, angle_span=2*np.pi, r_max=6.0, angle_offset=0.0, add_noise=False, noise_std=0.01):
        ...
        def scan(self, robot_state, env, return_ranges=False):
            # TODO implementare l'utilizzo del sensore nel progetto
            """
            Esegue una scansione dal robot_state = (x,y,theta).
            env: Environment
            return_ranges: se True restituisce (points, ranges), altrimenti solo points
            Ritorna: np.ndarray di forma (n_rays, 2) con coordinate world dei punti misurati (o r_max se nulla)
            """
            ...
            points = np.zeros((self.n_rays, 2))
            ranges = np.full((self.n_rays,), self.r_max, dtype=float)

            # pre-calcola union ostacoli per efficienza (Environment offre method)
            # Nota: env.first_intersection_with_line gestisce l'intersezione con tutti gli ostacoli
            for i, ang in enumerate(angles):
                end_x = x + self.r_max * np.cos(ang)
                end_y = y + self.r_max * np.sin(ang)
                ...
                if self.add_noise and ranges[i] < self.r_max:
                    noisy_r = max(0.0, ranges[i] + np.random.normal(0.0, self.noise_std))
                    ranges[i] = noisy_r
                    points[i, 0] = x + noisy_r * np.cos(ang)
                    points[i, 1] = y + noisy_r * np.sin(ang)
